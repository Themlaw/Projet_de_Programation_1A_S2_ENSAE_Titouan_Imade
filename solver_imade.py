...
class Solver:
    ...
    def cost(self, edge : tuple[tuple]) -> int: # returns the cost of the pair 
        (v1,v2) = edge
        return abs( self.grid.value[v1[0]][v1[1]] - self.grid.value[v2[0]][v2[1]] )
    
    def score(self, solution: list) -> int: # returns the score of self.pairs
        """
        Computes the of the list of pairs in self.pairs. !!!! Changed: returns the score of the solution 
        """
        s=0
        for edge in solution:
            (v1,v2) = edge
            if v2 == (-1,-1):
                s += self.grid.value[v1[0]][v1[1]]
            else:
                s += self.cost(edge)
        return s
        """for pair in self.pairs:
            (a,b) = pair
            if b == (-1,-1):
                s += 2 * self.grid.value[a[0]][a[1]] #2* car à la fin on divise par 2
            else:
                s += self.cost(pair)
        return s/2 """
...
def remove(pair : tuple, p : list) -> list: # removes the element pair in the list p 
    ...
    (a1,a2) = pair
    for (b1,b2) in p: 
        if b1 != a1 and b2 != a2 and b1 != a2 and b2 != a1:
            l.append((b1,b2))
    return l
...
class SolverGreedy(Solver):
    ...
    def glouton(self) -> list: # solves the grid with the greedy method : in each step, the less expensive pair is chosen 
        sol = []
        ...
        def interating(G, sol): # Iterates over a list
            if G == []: 
                ...
                for i in range(self.grid.n):
                    for j in range(self.grid.m):
                        if not d[(i,j)]: # autre idée : mettre cette methode avce A directement dans la fonction score, il faudra mettre le dico dans les arguments
                            A.append( ((i,j),(-1,-1)) ) # in this method, if the cell (i,j) is alone, we link it with a fake cell (-1,-1) and we add the edge ((i,j),(-1,-1)) to the solution for a score equal to the value of (i,j)
                #self.pairs=sol
                return  sol, self.score(A)
            if G != []:
                l = [self.cost(pair) for pair in G] 
                ind = self.index_min(l) # thus, l[ind] is the minimum of l 
                pair = G[ind]           # thus, pair is the less expensive edge
                d[pair[0]] = True
                d[pair[1]] = True
                sol.append(pair)
                return interating (remove(pair,G),sol) # we start again
        return interating(G,[])
...
class SolverBipart(Solver):
    ...
    def solution(self) -> tuple[list, int]: # returns the solution of the grid using a maximum matching 
        G = self.grid.all_pairs()
        C = []
        ...
        A = C.copy() # A is the set of all edges. An ((i,j),(-1,-1)) edge means that the cell (i,j) is not linked. We just want to see the real edge and therefore we keep C 
        for i in range(self.grid.n):
            for j in range(self.grid.m):
                if not (i,j) in df:
                    A.append (((i,j),(-1,-1))) 
        return "the solution is :", C, "and its score is: ", self.score(A)
